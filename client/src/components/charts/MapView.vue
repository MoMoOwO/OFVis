<template>
  ...
  >
    ...
    <!-- marker 图层 -->
    <l-marker
      :lat-lng="marker.length ? marker[2] : [0, 0]"
      v-show="!marker.length"
    >
      <l-tooltip>{{
        marker.length ? `Gradient: ${marker[1]}℃/km [${marker[2]}]` : [0, 0]
      }}</l-tooltip>
    </l-marker>
    <!-- markers 图层 -->
    ...
    <!-- gejson 图层 -->
    <l-geo-json
      :geojson="showRegionJson"
      :options="showRegionJsonOptions"
    ></l-geo-json>
  </l-map>
</template>
