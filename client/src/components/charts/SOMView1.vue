...
export default {
  ...
  methods: {
    ...
    async getClusterTreeData() {
      const { data: res } = await this.axios.get('/somv/clustertree')
    ...
    async getSOMResult() {
      ...
      const { data: res } = await this.axios.get('somv/somresult')
      if (res.meta.status !== 200) {
        ...
        // UMatrix series 数据
        this.uMatrixOpt.series[0].data = res.data.UMatrix // [x, y, dis, unitId] 添加外层聚类矩阵灰度图数据
        // 每个结点 样本统计数据
    ...
    // 获取权重图的数据
    /**
     * @prapm {Array, inde} WMatrix 后台返回的权重矩阵数据
     * @prapm {Array} indicatorArr 用于雷达图的指标数组
     * @return {Object} {radarSeries, }
     */
    // 获取权重矩阵图数据
