...
// 引入可以操作数据库的 Model 模块
const SOMDataModel = require('../modules/som-model-model');
...
router.put('/clustertree/:name', function (req, res, next) {
  ClusterTreeListModel.updateOne(req.params, req.body, (err, doc) => {
    if (err) {
      ...
      res.status(400).json({ meta: { msg: 'Update Cluster-Tree-List failed！', status: 400 } });
    } else {
      res.status(200).json({ data: 'Update Cluster-Tree-List Success!', meta: { msg: 'Update Cluster-Tree-List Success!', status: 200 } });
    }
  })
});
